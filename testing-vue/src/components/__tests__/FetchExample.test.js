...
it("can fetch users and reset them", async () => {
  const { queryByTestId, queryByText, getByText } = render(FetchExample, {
    props: { url: testUrl }
  });

  // no users loaded - button is aivable

  userEvent.click(getByText(/fetch.*users/i, { selector: "button" }));
  ...
  expect(queryByText(mockedResponse[0].name)).toBeInTheDocument();

  await userEvent.click(queryByText(/reset.*users/i, { selector: "button" }));
